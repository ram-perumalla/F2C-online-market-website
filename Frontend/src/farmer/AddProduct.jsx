...
const AddProduct = () => {
  ...
  const handleFile=(e)=>setImage(Array.from(e.target.files)); 
  const  handleUpdateSubmit=async(e)=>{
    e.preventDefault()
const formData= new FormData(e.target);
// formData.append("ProductImage", image);
image.forEach((file, index) => {
  formData.append(`ProductImage`, file); // Append each file under the same key or use `ProductImages[${index}]` for separate keys
});
const formProps= Object.fromEntries(formData);
console.log(formProps);
try {
const res=  await AxiosAPI.post(`farmer/product/${farmer._id}`, formData);
console.log(res);
...
//navigateTo("/")
  ...
  return (
    <div>
         <section className=" p-6  bg-green-800 rounded-md shadow-md dark:bg-gray-800 justify-center">
      ...
      <div className="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
        ...
        <div>
          ...
          {/* <select className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
           ...
           <input
            placeholder='In days e.g 3 days'
            type="text"
            name='shelflife'
            className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring"
          />
